...
export default class extends Component {
  ...
  handleLogin = () => {
    ...
    if (!(email && password)) {
      notification('error', 'Please enter a valid email and password');
      return;
    }
    this.setState({
      confirmLoading: true,
    ...
    Firebase.login(Firebase.EMAIL, { email, password })
      .catch(result => {
        const message =
          result && result.message ? result.message : 'An error occured';
  ...
  render() {
    return (
      <div>
        <Button type="primary" onClick={this.showModal} className="btnFacebook">
